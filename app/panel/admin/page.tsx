    ...
    <div className="min-h-screen w-full flex flex-col font-poppins bg-[var(--background)] text-[var(--foreground)] transition-colors pt-16 min-[375px]:pt-20 md:pt-28 no-scrollbar">
      ...
      {actaEdicion && (
        <div className="fixed inset-0 z-[150] bg-black/50 backdrop-blur-xs flex items-center justify-center p-3 no-scrollbar">
          <div className="bg-white rounded-3xl w-full max-w-lg p-4 min-[375px]:p-6 border border-[var(--border)] shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto no-scrollbar" style={{ scrollbarWidth: "none" }}>
            <div className="flex items-center justify-between border-b border-[var(--border)] pb-3">
              <div><h3 className="text-sm font-bold text-[var(--primary)]">Editar Partido #{actaEdicion.id}</h3><p className="text-[10px] opacity-60">Cancha {actaEdicion.fieldNumber} • Registrado por {actaEdicion.supervisorName}</p></div>
              <button onClick={() => setActaEdicion(null)} className="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-500 transition-colors flex items-center justify-center"><span className="material-symbols-outlined !text-[18px]">close</span></button>
            </div>
            <form onSubmit={guardarCambiosActa} className="space-y-4">
              <div className="p-3 rounded-2xl bg-gray-50 border border-[var(--border)] space-y-2">
                <label className="block text-[11px] font-bold opacity-70 uppercase tracking-wider text-center">Ajuste de Marcador Final</label>
              ...
              <div className="space-y-2">
                <label className="block text-[11px] font-bold opacity-70 uppercase tracking-wider">Eventos Registrados ({actaEdicion.events.length})</label>
                <div className="space-y-1.5 max-h-36 overflow-y-auto pr-0 no-scrollbar" style={{ scrollbarWidth: "none" }}>
                  {actaEdicion.events.map((ev) => (
                    <div key={ev.id} className="flex items-center justify-between p-2 rounded-xl bg-gray-50 border border-[var(--border)] text-xs">
                      <span>{ev.type === "GOL" ? "⚽" : ev.type === "AMARILLA" ? "🟨" : "🟥"} <strong>{ev.player}</strong> (Min {ev.minute}&apos;) - {ev.team === "A" ? actaEdicion.teamA : actaEdicion.teamB}</span>
                      <button type="button" onClick={() => eliminarEventoEnEdicion(ev.id)} className="text-red-500 hover:text-red-700"><span className="material-symbols-outlined !text-[16px]">delete</span></button>
                    </div>
                  ))}
                </div>
              </div>
              <div className="p-2.5 rounded-2xl bg-blue-50/50 border border-blue-100 text-[10px] space-y-1.5">
                ...
                <div className="grid grid-cols-3 gap-1">
                  <button type="button" onClick={() => agregarEventoEnEdicion("GOL", "A", 30, "Ajuste Admin")} className="py-1 bg-white border rounded-lg font-semibold hover:bg-gray-50">⚽ Gol {actaEdicion.teamA}</button>
                  <button type="button" onClick={() => agregarEventoEnEdicion("GOL", "B", 30, "Ajuste Admin")} className="py-1 bg-white border rounded-lg font-semibold hover:bg-gray-50">⚽ Gol {actaEdicion.teamB}</button>
                  <button type="button" onClick={() => agregarEventoEnEdicion("AMARILLA", "A", 30, "Jugador")} className="py-1 bg-white border rounded-lg font-semibold hover:bg-gray-50">🟨 Amarilla</button>
                </div>
              </div>
              <div>
                <label className="block text-[11px] font-bold opacity-70 uppercase tracking-wider mb-1">Notas u Observaciones del Partido (Incidentes)</label>
                <textarea rows={2} value={actaEdicion.incidentsNotes || ""} onChange={(e) => setActaEdicion({ ...actaEdicion, incidentsNotes: e.target.value })} className="w-full p-2 text-xs rounded-xl border border-[var(--border)] focus:outline-none focus:ring-1 focus:ring-[var(--primary)]" />
              </div>
              <div className="flex items-center justify-end gap-2 pt-2 border-t border-[var(--border)]">
                <button type="button" onClick={() => setActaEdicion(null)} className="px-4 py-2 rounded-xl text-xs font-semibold hover:bg-gray-100">Cancelar</button>
                <button type="submit" className="px-5 py-2 rounded-xl bg-[var(--primary)] text-white text-xs font-bold shadow-sm hover:opacity-90 transition-all flex items-center gap-1">Guardar Cambios</button>
              </div>
            </form>
          </div>
        </div>
